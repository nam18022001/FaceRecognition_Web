...
<table id="example" class="ui celled table" style="width:100%">
    ...
    <tbody>
        @foreach ($sinhvien as $value)
            <tr>
                <td>{{$value->id}}</td>
                <td>{{$value->Ten}} {{$value->Ho}} </td>
                <td>{{$value->MaSV}}</td>
                <td>
                    <div class="image-cropper">
                        <img src="{{ asset('sinhvien_avatars') }}/{{$value->avatar}}" class="rounded" />
                     </div>
                </td>
                <td>
                    <a class="btn btn-outline-warning btn-rounded" href="{{url('admin/sinhvien/sua')}}/{{$value->id}}">Sửa</a>
                    <a class="btn btn-outline-danger btn-rounded " href="{{url('admin/sinhvien/xoa')}}/{{$value->id}}">Xóa</a>
                </td>
            </tr>
